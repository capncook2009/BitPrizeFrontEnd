...
export const withdrawFormTokenAddressAtom = atom<Address | undefined>(
  undefined
);
...
export const WithdrawForm = (props: WithdrawFormProps) => {
  ...
  //: todo fix exchange rates for usdc or other assets
  ...
  const { data: tokenWithAmount, isFetched: isFetchedTokenBalance } =
    useTokenBalance(vault.chainId, userAddress!, tokenAddress!, {
      refetchOnWindowFocus: true,
    });
  const tokenBalance =
    isFetchedTokenBalance && !!tokenWithAmount ? tokenWithAmount.amount : 0n;

  const { data: vaultBalance, isFetched: isFetchedVaultBalance } =
    useUserVaultShareBalance(vault, userAddress!);
  const shareBalance =
    isFetchedVaultBalance && !!vaultBalance ? vaultBalance.amount : 0n;
  ...
  const [formShareAmount, setFormShareAmount] = useAtom(
  ...
  const handleShareAmountChange = (shareAmount: string) => {
    if (
      ...
      console.log("input test ", shareAmount);
  ...
  useEffect(() => {
    if (
      !!share &&
      !!tokenData &&
      isValidFormInput(formShareAmount, share.decimals)
    ) {
  ...
  const tokenPickerOptions = useMemo(() => {
    const getOptionId = (option: Token) =>
      `zapToken-${option.chainId}-${option.address}`;
    ...
    if (!!vaultToken) {
      const isVaultToken = (id: string) =>
        lower(id.split("-")[2]) === lower(vaultToken.address);

      const vaultTokenOption = options.find((option) =>
        isVaultToken(option.id)
      );
      ...
      if (!!vaultTokenOption) {
        ...
        const value =
          parseFloat(formatUnits(amount, vaultToken.decimals)) * price;

        options.unshift({
          id: getOptionId(vaultToken),
          content: (
            <TokenPickerOption
              token={{ ...vaultToken, amount, price, value }}
            />
          ),
  ...
  useEffect(() => {
    if (
      ...
      const inputValue =
        parseFloat(formatUnits(withdrawAmount, shareInputData.decimals)) *
        shareInputData.price;
      const outputValue =
        parseFloat(
          formatUnits(zapAmountOut.expected, tokenInputData.decimals)
  ...
  const handleRange = (value: any) => {
    ...
    //
    if (!shareInputData?.decimals) {
      return;
    }
    const shareBalance = shareInputData?.amount.toString();
    const inputShare: any = getPercentageFormatted(shareBalance, value);
    console.log("input test ", { shareBalance, inputShare });
    handleShareAmountChange(formatUnits(inputShare, shareInputData?.decimals));
  };
        ...
        <TxFormInput
          ...
          validate={{
            isNotGreaterThanShareBalance: (v) =>
              (!!shareInputData &&
                parseFloat(
                  formatUnits(shareInputData.amount, shareInputData.decimals)
                ) >= parseFloat(v)) ||
              !isFetchedVaultBalance ||
              !vaultBalance ||
              t_errors("notEnoughTokens", {
                symbol: shareInputData?.symbol ?? "?",
              }),
          }}
