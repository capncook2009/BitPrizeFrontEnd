...
export const WithdrawTxButton = (props: WithdrawTxButtonProps) => {
  ...
  const { userDeposits, refetch } = useVaultDataReader(userAddress, vault);
  ...
  const withdrawAmount = userDeposits;
  ...
  const {
    ...
    withdrawTxHash,
    sendRedeemTransaction,
  } = useVaultWithdraw(vault.address);
  ...
  if (withdrawAmount === 0n) {
    ...
    return (
      <TransactionButton
        ...
        write={() => sendRedeemTransaction(withdrawAmount)}
        txHash={withdrawTxHash}
        txDescription={t_modals("withdrawTx", {
