...
export const SuccessView = (props: SuccessViewProps) => {
  ...
  const formattedSharesReceived =
    !!share && !!sharesReceived
      ? formatBigIntForDisplay(sharesReceived, share.decimals, {
...
const ShareButtons = (props: ShareButtonsProps) => {
  ...
  const hashTags = ["BitPrize", network];
...
const getShareText = (tokenSymbol: string, platform: SharePlatform) => {
  ...
  const textOptions: string[] = [
    ...
    `🎉 Just tried something exciting and new! Joined a fun savings game #BitPrize where everyone has a chance to win big rewards! 🤞 Feeling lucky and ready for adventure! 🏆✨`,
