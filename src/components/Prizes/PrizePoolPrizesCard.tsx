...
export const PrizePoolPrizesCard = (props: PrizePoolPrizesCardProps) => {
  ...
  const { data: allPrizeInfo, isFetched: isFetchedAllPrizeInfo } =
    useAllPrizeInfo([prizePool]);
  const { data: tokenData, isFetched: isFetchedTokenData } =
    usePrizeTokenData(prizePool);
  ...
  return (
    <Card
      ...
      className={classNames(
        "gap-3 items-center !justify-start md:gap-4",
        innerClassName
      )}
    >
      <NetworkBadge
      ...
      >
        <span className="flex-grow pl-8 text-left md:pl-16">
          {t_prizes("prize")}
        </span>
        <span className="flex-grow pr-8 text-right md:pr-16">
          {t_prizes("frequency")}
        </span>
      </div>
      {isFetchedAllPrizeInfo &&
      isFetchedTokenData &&
      !!tokenData &&
      !!drawPeriod ? (
        <div className="w-full flex flex-col gap-3">
          {Object.values(allPrizeInfo)[0]
            .slice(0, -2)
            .map((prize, i) => {
              const frequency = formatDailyCountToFrequency(
                prize.dailyFrequency,
                {
