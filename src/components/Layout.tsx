...
export const Layout = (props: LayoutProps) => {
  ...
  const { refetch: refetchUserVaultDelegationBalances } =
  ...
  const pageTitle =
    overrides?.pageTitle ?? pageTitles[router.pathname.split("/")[1]];

  return (
    <div className="flex flex-col min-h-screen">
      <Head>
        <title>{`BitPrize App${!!pageTitle ? ` | ${pageTitle}` : ""}`}</title>
      </Head>
      ...
      <DepositModal
        ...
        onSuccessfulApproval={() =>
          fathom.trackEvent(FATHOM_EVENTS.approvedExact)
        }
        onSuccessfulDeposit={(chainId, txReceipt) => {
          fathom.trackEvent(FATHOM_EVENTS.deposited);
          !!walletId &&
            trackDeposit(chainId, txReceipt.transactionHash, walletId);
        }}
      ...
      <WithdrawModal
        ...
        onSuccessfulWithdrawalWithZap={() =>
          fathom.trackEvent(FATHOM_EVENTS.redeemedWithZap)
        }
      />

      <DelegateModal
        onSuccessfulDelegation={() =>
          fathom.trackEvent(FATHOM_EVENTS.delegated)
        }
      />
      ...
      <CheckPrizesModal
        onWin={() =>
          fathom.trackEvent(FATHOM_EVENTS.checkedPrizes, { _value: 1 })
        }
        onNoWin={() =>
          fathom.trackEvent(FATHOM_EVENTS.checkedPrizes, { _value: 0 })
        }
      />
