...
export async function POST(
  ...
  const postUrl = `${DOMAINS.hackathon}/api/frame/account/${params.user}`;
  ...
  if (!prevState) {
    ...
    if (!!userAddress) {
      return frame(postUrl, undefined, {
        user: { name: params.user, address: userAddress },
      });
    } else {
...
const frame = (
  postUrl: string,
  prevState: FrameState | undefined,
  data: FrameData
) => {
  ...
  return frameResponse<FrameState>({
    ...
    buttons: [
      { content: "Check Wins" },
      {
        content: "View on App",
        action: "link",
        target: `${DOMAINS.hackathon}/account/${user.name}`,
      },
    ],
