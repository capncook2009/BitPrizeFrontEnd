...
export const frameResponse = <FrameStateType extends {}>(data: {
  ...
  if (!!state) {
    frame += `<meta name='fc:frame:state' content='${JSON.stringify(
      state
    )}' />`;
  }
  ...
  buttons.forEach((button, i) => {
    frame += `<meta name='fc:frame:button:${i + 1}' content='${
      button.content
    }' />`;

    if (!!button.action) {
      frame += `<meta name='fc:frame:button:${i + 1}:action' content='${
        button.action
      }' />`;
      ...
      if (
        ...
        frame += `<meta name='fc:frame:button:${i + 1}:target' content='${
          button.target
        }' />`;

        if (button.action === "tx") {
          frame += `<meta name='fc:frame:button:${i + 1}:post_url' content='${
            button.callback
          }' />`;
        }
      }
...
export const imageResponse = async (
  ...
  const fontUrl = `${DOMAINS.hackathon}/fonts/inter/inter-regular.woff`;
...
export const getUserVaultBalances = async (
  ...
  const userVaultBalances: {
    [tokenAddress: Address]: TokenWithAmount & VaultInfo;
  } = {};
  ...
  Object.entries(tokens).forEach(([_tokenAddress, token]) => {
    ...
    const vaultInfo = validVaults.find(
      (v) => getVaultId(v) === getVaultId(token)
    );
...
export const getUserLastPrizes = async (
  network: NETWORK,
  userAddress: Address
) => {
  const userLastPrizes = await getUserSubgraphPrizes(network, userAddress, {
    numPrizes: 10,
  });
