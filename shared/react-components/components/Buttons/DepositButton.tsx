...
export interface DepositButtonProps
  ...
  intl?: {
    base: Intl<"deposit">;
    tooltips: Intl<
      "deprecatedVault" | "wrongPrizePoolVault" | "badPrecisionPerDollar"
    >;
  };
}
...
export const DepositButton = (props: DepositButtonProps) => {
  ...
  const isWrongPrizePool = useMemo(() => {
    if (!!vaultTwabController) {
      const prizePoolTwabController = PRIZE_POOLS.find(
        (pool) => pool.chainId === vault.chainId
      )?.options.twabControllerAddress;

      if (!!prizePoolTwabController) {
        return (
          vaultTwabController.toLowerCase() !==
          prizePoolTwabController.toLowerCase()
        );
      }
    }
  }, [vaultTwabController]);
  ...
  if (isBadPrecisionPerDollar) {
    return (
      <BadPrecisionPerDollarTooltip
        intl={intl?.tooltips("badPrecisionPerDollar")}
      >
        <Button
          onClick={handleClick}
          className={buttonClassName}
